...
@WebServlet("/userAddRegistration")
public class userAddRegistrationServlet extends HttpServlet {
    ...
    private void insertRegistration(HttpServletRequest request, HttpServletResponse response)
        ...
        registration regis = new registration(grp_name, cptn_name, cptn_email, cptn_number, mem_one_name, mem_two_name, mem_three_name,
                mem_four_name, mem_five_name, mem_six_name, mem_seven_name, code, date);

        regisdao.insertRegistration(regis);
        response.sendRedirect("event-calendar.jsp");
